export function renderPlayerCards({d,scene,deps}){
  ...
  return actors.map((a,i)=>{
    ...
    const ownerBtns=!ce&&pinOwner
      ...
      : "";
    return `<div class="${cls}" data-idx="${i}" data-actor-id="${a.id||""}" draggable="true"><div class="totm-card-bg" style="background-image:url('${bg}');background-position:${a.bgOffsetX??50}% ${a.bgOffsetY??20}%;background-size:${a.bgScale??150}%"></div><div class="totm-card-overlay"></div>${overlay}${fp!=null?`<div class="totm-fp-badge" title="Fabula Points"><span class="totm-fp-label">FP</span><span class="totm-fp-value">${fp}</span></div>`:""}${ac.length?`<div class="totm-conditions">${ac.map(c=>{const df=conds.find(x=>x.id===c);return df?`<span class="totm-condition-badge ${df.color}"><i class="${df.icon}"></i> ${df.label}</span>`:"";}).join("")}</div>`:""}${(gmBtns||ownerBtns)?`<div class="totm-actor-btns">${gmBtns||ownerBtns}</div>`:""}<div class="totm-card-content"><div class="totm-actor-name">${a.name}</div>${sub?`<div class="totm-actor-subtitle">${sub}</div>`:""}${renderBars(res,{kind:"player"})}</div>${((io&&canAfk)||ce)?`<div class="totm-actor-status-bar">${(io&&canAfk)?`<button data-status="afk" class="${status==="afk"?"active-status":""}">AFK</button>`:""} ${ce?`<button data-status="targeted" class="${status==="targeted"?"active-status":""}">TGT</button><button data-status="missing" class="${status==="missing"?"active-status":""}">MIA</button><button data-act="conditions"><i class="fas fa-list"></i></button>`:""}</div>`:""}</div>`;
  }).join("");
}
